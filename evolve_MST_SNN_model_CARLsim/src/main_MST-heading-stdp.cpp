...
public:
	...
	void run(const ParameterInstances &parameters, std::ostream &outputStream) const {
		...
		gMT = network->createSpikeGeneratorGroup("MT", MTDim, EXCITATORY_POISSON, 1, CPU_CORES); // 1, GPU_CORES); //input
		for (unsigned int i = 0; i < numIndi; i++) {
			
			// creat neuron groups
			gMST[i] = network->createGroup("MST", MSTDim, EXCITATORY_NEURON, 0, CPU_CORES); // GPU_CORES
			gInh[i] = network->createGroup("inh", inhDim, INHIBITORY_NEURON, 0, CPU_CORES); // GPU_CORES
...
int main(int argc, char* argv[]) {
	...
	const SimMode simMode = CPU_MODE; // CPU_MODE;
